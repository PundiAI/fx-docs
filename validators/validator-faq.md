...
* **Tendermint Key**: This is a unique key used to sign consensus votes.
  * It is associated with a public key `fxvalconspub` (Get this value with `fxcored tendermint show-validator`)
  * It is generated when the node is created with fxcored init.
* **Application key**: This key is created from `fxcored` and used to sign transactions. Application keys are associated with a public key prefixed by `fxpub` and an address prefixed by `fx`. Both are derived from account keys generated by `fxcored keys add`.

Note: A validator's operator key is directly tied to an application key, but uses reserved prefixes solely for this purpose: `fxvaloper` and `fxvaloperpub`
