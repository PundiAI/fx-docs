...
fxCore also supports the Ethereum [JSON-RPC](https://eth.wiki/json-rpc/API) filters calls to
...
```bash
fxcored start --rpc.laddr="tcp://127.0.0.1:26657"
```
...
Since fxCore runs uses Tendermint Core as it's consensus Engine and it's built with the Cosmos
...
PubSubAPI](https://geth.ethereum.org/docs/rpc/pubsub), fxCore needs to cast the Tendermint
...
```bash
fxcored start  --json-rpc.address"0.0.0.0:8545" --json-rpc.ws-address="0.0.0.0:8546" --evm.rpc.api="eth,web3,net,txpool,debug" --json-rpc.enable
```
